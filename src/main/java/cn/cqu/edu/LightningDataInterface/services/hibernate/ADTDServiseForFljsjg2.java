...
public class ADTDServiseForFljsjg2 extends BaseService {
	...
	public String GetLightningDensityForWholeProvice(int year, int radium)
		...
		BasePath = new FileTools().GetProjectRootPath() + "resources" + separator + "json" + separator
				+ "thunderDistribute" + separator;// 设置路径地址
	...
	/**
	 * 返回给定时间段闪电活动数据 但由于adtdService中listbyDate的问题，返回的是StartTime和EndTime之间全天日期的数据
	 ...
	 * 
	 * @param AreaIndex
	 *            0-城口县 1-巫溪县 2- 巫山县 3-开县 4-云阳县 5-奉节县 6-万州区 7-梁平县 8-忠县 9-石柱县
	 *            10-垫江县 11-丰都县 12-彭水县 13-黔江区 14-酉阳县 15-秀山县 16-大渡口区 17-武隆县
	 *            18-涪陵区 19-长寿区 20-南川区 21-万盛经开区 22-綦江区 23-江津区 24-璧山区 25-北碚区
	 *            26-渝北区 27-巴南区 28-南岸区 29-江北区 30-合川区 31-沙坪坝区 32-九龙坡区 33-荣昌区
	 *            34-永川区 35-大足区 36-铜梁区 37-潼南县 38-渝中区 39-双桥区 40-重庆全市
	 * @param StartDate
	 * @param EndDate
	 * @throws IOException
	 * @throws NumberFormatException
	 ...
	 */
	@Transactional
	public void GetLightningAreaStatisticByDate(int AreaIndex, Date StartDate, Date EndDate)
			throws IOException, NumberFormatException, SQLException {
		...
		for (long day = 0; day < Days; day++) {
			...
			// 判断文件是否生成
			String path = new FileTools().GetProjectRootPath() + "resources" + separator + "json" + separator
					+ "thunderActive" + separator + "area-" + AreaIndex + "-date-" + dateString + ".json";
			File file = new File(path);
			if (file.exists()) // 文件存在
			{
			...
			String aString2 = new FileTools()
					.ReadFileAsString((new FileTools().GetProjectRootPath() + "resources" + separator + "area.json"));// 获取本地文件里的区域信息
			...
			fos.write(title);
			fos.write("\r\n");
			String title2 = "{" + "\"areaid\":" + AreaTemp.getfId() + ",\"name\":\"" + AreaTemp.getName() + "\","
			...
			for (int section_index = 0; section_index < 24; section_index++) {
				...
				String str = "{\"section\":" + section_index + "," + "\"starttime\":\"" + StartTime + "\","
						+ "\"endtime\":\"" + EndTime + "\"," + "\"positions\":[";
				fos.write(str);
				fos.write("\r\n");
				...
				for (int data_index = 0; data_index < ADTD.size(); data_index++) {
					...
					if ((int) (section_index) == ADTD.get(data_index).getHour()) {
						...
						if (MapTools.isInPolygon(PointForCheck, AreaBoundaryArray)) {
							...
							String str1 = "{\"latitude\":" + ADTD.get(data_index).getLatitude() + "," + "\"longitude\":"
									+ ADTD.get(data_index).getLongitude() + "," + "\"intensity\":"
									+ ADTD.get(data_index).getIntensity() + ",\"datetime\":"
									+ Datecalendar.getTime().getTime() + "}";
							fos.write(str1);
							...
							AreaMaxIntensity = Math.max(AreaMaxIntensity,
									Math.abs(ADTD.get(data_index).getIntensity().floatValue()));
							AreaPositiveMaxIntensity = Math.max(AreaPositiveMaxIntensity,
									ADTD.get(data_index).getIntensity().floatValue());
							AreaNegativeMaxIntensity = Math.min(AreaNegativeMaxIntensity,
									ADTD.get(data_index).getIntensity().floatValue());

							MaxIntensity = Math.max(MaxIntensity,
									Math.abs(ADTD.get(data_index).getIntensity().floatValue()));
							PositiveMaxIntensity = Math.max(PositiveMaxIntensity,
									ADTD.get(data_index).getIntensity().floatValue());
							NegativeMaxIntensity = Math.min(NegativeMaxIntensity,
									ADTD.get(data_index).getIntensity().floatValue());
						}
					}
				}
				...
				if (section_index == 23)// 最后一个时间段，行尾加反方括号和逗号
					title3 = "\"LightningCount\":" + count + ",\"PositiveCount\":\"" + PositiveCount + "\","
							+ "\"NegativeCount\":" + NegativeCount + "," + "\"MaxIntensity\":" + MaxIntensity + ","
							+ "\"PositiveMaxIntensity\":" + PositiveMaxIntensity + "," + "\"NegativeMaxIntensity\":"
							+ NegativeMaxIntensity + "}],";
				else// 中间时间段，行尾加逗号
					title3 = "\"LightningCount\":" + count + ",\"PositiveCount\":\"" + PositiveCount + "\","
							+ "\"NegativeCount\":" + NegativeCount + "," + "\"MaxIntensity\":" + MaxIntensity + ","
							+ "\"PositiveMaxIntensity\":" + PositiveMaxIntensity + "," + "\"NegativeMaxIntensity\":"
			...
			{
				title4 = "\"AreaMaxIntensity\":" + AreaMaxIntensity + "," + "\"AreaPositiveMaxIntensity\":"
						+ AreaPositiveMaxIntensity + "," + "\"AreaNegativeMaxIntensity\":" + AreaNegativeMaxIntensity
						+ ",\"AreaLightningCount\":\"" + AreaLightningCount + "\"," + "\"AreaPositiveCount\":"
						+ AreaPositiveCount + "," + "\"AreaNegativeCount\":" + AreaNegativeCount
						+ ",\"LightingStartTime\":\"" + DateConvert.DateToStr(LightingStartTime) + "\","
						+ "\"LightingEndTime\":\"" + DateConvert.DateToStr(LightingEndTime) + "\"";
			} else// 当天无雷电数据时,雷电初时和雷电终时设置为0
			{
				title4 = "\"AreaMaxIntensity\":" + AreaMaxIntensity + "," + "\"AreaPositiveMaxIntensity\":"
						...
						+ ",\"AreaLightningCount\":\"" + AreaLightningCount + "\"," + "\"AreaPositiveCount\":"
						+ AreaPositiveCount + "," + "\"AreaNegativeCount\":" + AreaNegativeCount
						+ ",\"LightingStartTime\":\"" + 0 + "\"," + "\"LightingEndTime\":\"" + 0 + "\"";
			}
			fos.write(title4);
			fos.write("\r\n");
			// 结束写文件
	 ...
	 */
	@Transactional
	public LigthingActiveStatistic_Month_Year GetLightningAreaStatisticByMonth(int AreaIndex, Date Month)
			throws NumberFormatException, IOException, SQLException, ParseException {
		...
		for (long day = 0; day < days; day++) {
			...
			String path = new FileTools().GetProjectRootPath() + "resources" + separator + "json" + separator
					+ "thunderActive" + separator + "area-" + AreaIndex + "-date-" + Dateformatter.format(date)
					+ ".json";
			...
			if (foo[0].getAreaLightningCount() > 0 && DateConvert.StrToDate(LightingStartTime).getTime() >= DateConvert
					.StrToDate(foo[0].getLightingStartTime()).getTime())// 确定当月雷电初时
			{
				LightingStartTime = foo[0].getLightingStartTime();
			}

			if (foo[0].getAreaLightningCount() > 0 && DateConvert.StrToDate(LightingEndTime).getTime() <= DateConvert
					.StrToDate(foo[0].getLightingEndTime()).getTime())// 确定当月雷电终时
			{
	 ...
	 */
	@Transactional
	public LigthingActiveStatistic_Month_Year GetLightningAreaStatisticByYear(int AreaIndex, Date Year)
			throws NumberFormatException, IOException, SQLException, ParseException {
		...
		for (int month = 0; month <= Months; month++) {
			...
			if (MonthStatistic.getAreaLightningCount() > 0 && DateConvert.StrToDate(LightingStartTime)
					.getTime() >= DateConvert.StrToDate(MonthStatistic.getLightingStartTime()).getTime())// 确定当月雷电初时
			{
				LightingStartTime = MonthStatistic.getLightingStartTime();
			}

			if (MonthStatistic.getAreaLightningCount() > 0 && DateConvert.StrToDate(LightingEndTime)
					.getTime() <= DateConvert.StrToDate(MonthStatistic.getLightingEndTime()).getTime())// 确定当月雷电终时
			{
	...
	public String GetLightingCricleStatisticByDate(double CricleLatitude, double CricleLongitude, float Radium,
		...
		List<ADTD> TempADTD = adtdService.listbyDateAndArea(Date, Date, latitudeLower, latitudeUpper, longitudeLeft,
				longitudeRight);

		String JsonString;

		for (int i = 0; i < TempADTD.size(); i++) {
			if (MapTools.GetDistance(CricleLongitude, CricleLatitude, TempADTD.get(i).getLongitude(),
					TempADTD.get(i).getLatitude()) <= Radium) {
		...
		// 开始写JSON
		JsonString = "[" + "";
		JsonString += "{" + "\"CricleLatitude\":" + CricleLatitude + ",\"CricleLongitude\":" + CricleLongitude
				+ ",\"Radium\":" + Radium / 1000 + "," + "\"date\":\"" + Dateformatter.format(Date) + "\","
				+ "\"time\":" + "[";
	 ...
	 * @return
	 */
	public LigthingCricleActiveStatistic_Month_Year GetLightningCricleStatisticByMonth(double CricleLatitude,
			double CricleLongitude, float Radium, Date Month) {
		Date FirstDay = Month; // 获取Month的第一天
		Calendar cal = Calendar.getInstance();// 获取当前日期
		cal.setTime(Month);
		// 为了获得Month的最后一天，首先将月份加1，之后将天数减1，便能得到Month的最后一天
		...
		for (long day = 0; day < days; day++) {
			...
			LigthingCricleActiveStatistic_Day foo[] = new Gson().fromJson(jsonData,
					LigthingCricleActiveStatistic_Day[].class);// 将json文本信息转换为对象
			TimePeriod temp = new TimePeriod();
	 ...
	 */
	@Transactional
	public LigthingCricleActiveStatistic_Month_Year GetLightningCricleStatisticByYear(double CricleLatitude,
			double CricleLongitude, float Radium, Date Year) {
		...
		for (int month = 0; month <= Months; month++) {
			...
			LigthingCricleActiveStatistic_Month_Year MonthStatistic = GetLightningCricleStatisticByMonth(CricleLatitude,
					CricleLongitude, Radium, date);// 获取一个月的统计信息
	...
	public String GetLightingRectangleStatisticByDate(double latitudeLower, double latitudeUpper, double longitudeLeft,
			double longitudeRight, Date Date) {
		List<ADTD> ADTD = adtdService.listbyDateAndArea(Date, Date, (float) latitudeLower, (float) latitudeUpper,
				(float) longitudeLeft, (float) longitudeRight);

		// 开始写JSON
		String JsonString = "[" + "";
		JsonString += "{" + "\"latitudeLower\":" + latitudeLower + ",\"latitudeUpper\":" + latitudeUpper
				+ ",\"longitudeLeft\":" + longitudeLeft + ",\"longitudeRight\":" + longitudeRight + "," + "\"date\":\""
	 ...
	 * @return
	 */
	public LigthingRectangleActiveStatistic_Month_Year GetLightningRectangleStatisticByMonth(double latitudeLower,
			double latitudeUpper, double longitudeLeft, double longitudeRight, Date Month) {
		Date FirstDay = Month; // 获取Month的第一天
		Calendar cal = Calendar.getInstance();// 获取当前日期
		cal.setTime(Month);
		// 为了获得Month的最后一天，首先将月份加1，之后将天数减1，便能得到Month的最后一天
		...
		for (long day = 0; day < days; day++) {
			...
			String jsonData = GetLightingRectangleStatisticByDate(latitudeLower, latitudeUpper, longitudeLeft,
					longitudeRight, date);// 取出单独一天的文件数据
			LigthingRectangleActiveStatistic_Day foo[] = new Gson().fromJson(jsonData,
					LigthingRectangleActiveStatistic_Day[].class);// 将json文本信息转换为对象
			TimePeriod temp = new TimePeriod();
	 ...
	 */
	@Transactional
	public LigthingRectangleActiveStatistic_Month_Year GetLightningRectangleStatisticByYear(double latitudeLower,
			double latitudeUpper, double longitudeLeft, double longitudeRight, Date Year) {
		...
		for (int month = 0; month <= Months; month++) {
			...
			LigthingRectangleActiveStatistic_Month_Year MonthStatistic = GetLightningRectangleStatisticByMonth(
					latitudeLower, latitudeUpper, longitudeLeft, longitudeRight, date);// 获取一个月的统计信息
	...
	private void GenerateOneYearData(int year) throws IOException, NumberFormatException, SQLException {
		...
		for (int radium = 1; radium <= 20; radium++) {
			...
			for (int i = 0; i < allArea.length - 1; i++)// 除开allArea的最后一个区域：整个重庆市
			{
				tempArea = allArea[i];
				// 确定该区域需要的方格数
				int allOverallRow = (int) (((tempArea.getLatitudeUpper() - tempArea.getLatitudeLower()) * 111.0
						/ radium) + 1);// 默认认为1°的纬度为111公里
				int allOverallCol = (int) (((tempArea.getLongitudeRight() - tempArea.getLongitudeLeft()) * 96.0
						/ radium) + 1);// 默认认为1°的经度为96公里
				int location[][] = new int[allOverallRow][allOverallCol];
				// 计算该区域的面积（矩形面积）
				double size = (tempArea.getLatitudeUpper() - tempArea.getLatitudeLower()) * 111.0
						* (tempArea.getLongitudeRight() - tempArea.getLongitudeLeft()) * 96.0;
				// 确定每个方格里的闪电次数
				for (int LightningIndex = 0; LightningIndex < ADTD.size(); LightningIndex++) {
					...
					if (ADTD.get(LightningIndex).getLongitude() <= tempArea.getLongitudeRight()
						...
						// 判断该闪电在哪个方格里
						int disX = (int) ((Math
								.abs(ADTD.get(LightningIndex).getLatitude() * 111 - tempArea.getLatitudeUpper() * 111))
								/ radium);
						int disY = (int) ((Math
								.abs(ADTD.get(LightningIndex).getLongitude() * 96 - tempArea.getLongitudeLeft() * 96))
								/ radium);
						location[disX][disY]++; // 若在该方格里，该方格里的闪电数加1
					}
				}
				// 开始写区域信息
				String title2 = "{" + "\"areaid\":" + tempArea.getfId() + ",\"name\":\"" + tempArea.getName() + "\","
						+ "\"radium\":" + radium + "," + "\"matrixs\":" + "[";
				fos.write(title2);
				fos.write("\r\n");
				int index = 0;
				int CountLighting = 0;
				for (int d = 0; d < allOverallRow; d++) {
					for (int b = 0; b < allOverallCol; b++) {
						...
						if ((b + 1) * (d + 1) == allOverallRow * allOverallCol)// 如果到该区域数据的最后一行，行尾不加逗号
							str = "{\"serial\":" + index + "," + "\"LongitudeLeft\":" + LongitudeLeft + ","
									+ "\"LatitudeUpper\":" + LatitudeUpper + "," + "\"LongitudeRight\":"
									+ LongitudeRight + "," + "\"LatitudeLower\":" + LatitudeLower + ",\"count\":"
									+ location[d][b] + ",\"LightningDensity\":"
									+ location[d][b] * 1.0f / (radium * radium * 1.0f) + "}";
						else// 数据的中间行，行尾加逗号
							str = "{\"serial\":" + index + "," + "\"LongitudeLeft\":" + LongitudeLeft + ","
									+ "\"LatitudeUpper\":" + LatitudeUpper + "," + "\"LongitudeRight\":"
									+ LongitudeRight + "," + "\"LatitudeLower\":" + LatitudeLower + ",\"count\":"
				...
				if (i == allArea.length - 2)// 最后一个区域，行尾不加逗号，并且除开allArea的最后一个区域：整个重庆市
					fos.write("],\"CountLighting\":" + CountLighting + ",\"AreaLightningDensity\":"
							+ CountLighting * 1.0f / (size) + "}");// 统计总闪电次数和闪电密度
				else// 中间区域，行尾加逗号
					fos.write("],\"CountLighting\":" + CountLighting + ",\"AreaLightningDensity\":"
							+ CountLighting * 1.0f / (size) + "},");
				fos.write("\r\n");
			}
			fos.write("]}");
			fos.close();
	...
	/**
	 * 生成圆形或矩形区域内按天的雷电统计数据 由于圆形和矩形的区域表示方法不同故json头部的圆形和矩形信息需要由JsonString传入
	 * 
	 * @param JsonString
	 * @param ADTD
	 * @return
	...
	private String GenJsonForLigRect_CricleActiveStatisticByDate(String JsonString, List<ADTD> ADTD) {
		...
		for (int section_index = 0; section_index < 24; section_index++) {
			...
			String StartTime = section_index + ":" + "00";
			String EndTime = section_index + 1 + ":" + "00";
			JsonString += "{\"section\":" + section_index + "," + "\"starttime\":\"" + StartTime + "\","
					+ "\"endtime\":\"" + EndTime + "\"," + "\"positions\":[";
			...
			for (int data_index = 0; data_index < ADTD.size(); data_index++) {
				...
				if ((int) (section_index) == ADTD.get(data_index).getHour()) {
					...
					JsonString += "{\"latitude\":" + ADTD.get(data_index).getLatitude() + "," + "\"longitude\":"
							+ ADTD.get(data_index).getLongitude() + "," + "\"intensity\":"
							+ ADTD.get(data_index).getIntensity() + ",\"datetime\":" + Datecalendar.getTime().getTime()
							+ "}";
			...
			// 写入时间段统计信息
			if (section_index == 23)// 最后一个时间段，行尾加反方括号和逗号
				JsonString += "\"LightningCount\":" + count + ",\"PositiveCount\":\"" + PositiveCount + "\","
						+ "\"NegativeCount\":" + NegativeCount + "," + "\"MaxIntensity\":" + MaxIntensity + ","
						+ "\"PositiveMaxIntensity\":" + PositiveMaxIntensity + "," + "\"NegativeMaxIntensity\":"
						+ NegativeMaxIntensity + "}],";
			else// 中间时间段，行尾加逗号
		...
		if (ADTD.size() > 0)// 当天有雷电数据时
		{
			JsonString += "\"AreaMaxIntensity\":" + AreaMaxIntensity + "," + "\"AreaPositiveMaxIntensity\":"
					+ AreaPositiveMaxIntensity + "," + "\"AreaNegativeMaxIntensity\":" + AreaNegativeMaxIntensity
					+ ",\"AreaLightningCount\":\"" + AreaLightningCount + "\"," + "\"AreaPositiveCount\":"
					+ AreaPositiveCount + "," + "\"AreaNegativeCount\":" + AreaNegativeCount
					+ ",\"LightingStartTime\":\"" + DateConvert.DateToStr(LightingStartTime) + "\","
