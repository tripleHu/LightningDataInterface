...
public class SyncAdtdTask {
	...
	public void UpdateAdtd() {
		...
		while (!(StartDatecalendar.get(Calendar.YEAR) == Nowcalendar.get(Calendar.YEAR)
			...
			try {
				...
				// 最长1小时
				String[] result = (String[]) _hmsasserverPort.getflashdata("FLASH_LIGHT_DATA", StartSection, EndSection,
						"*", "*", "cqu", "123456");
				if (result.length > 1) {
	...
	private boolean insertDate(String result) {
		...
		if (lines.length == 10) {
			...
			String tsStr = strDate.substring(0, 4) + "-" + strDate.substring(4, 6) + "-" + strDate.substring(6, 8) + " "
					+ "00:00:00";
